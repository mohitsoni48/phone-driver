...
tap_on() {
    ...
    if [ ! -f /tmp/phonedriver_before.xml ]; then
    ...
    adb_cmd shell rm /sdcard/phonedriver_after.xml 2>/dev/null

    if [ ! -f /tmp/phonedriver_after.xml ]; then
        echo "MEMO_SKIP: Could not capture post-tap screen"
        return 0
    fi
